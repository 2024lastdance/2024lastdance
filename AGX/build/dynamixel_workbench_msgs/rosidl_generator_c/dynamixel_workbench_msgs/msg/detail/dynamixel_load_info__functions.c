  ...
  dynamixel_workbench_msgs__msg__DynamixelLoadInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    dynamixel_workbench_msgs__msg__DynamixelLoadInfo * data =
      (dynamixel_workbench_msgs__msg__DynamixelLoadInfo *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!dynamixel_workbench_msgs__msg__DynamixelLoadInfo__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          dynamixel_workbench_msgs__msg__DynamixelLoadInfo__fini(&data[i]);
        }
        free(data);
        return false;
      }
