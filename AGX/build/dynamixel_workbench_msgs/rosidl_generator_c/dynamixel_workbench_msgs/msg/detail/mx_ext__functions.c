  ...
  dynamixel_workbench_msgs__msg__MXExt__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    dynamixel_workbench_msgs__msg__MXExt * data =
      (dynamixel_workbench_msgs__msg__MXExt *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!dynamixel_workbench_msgs__msg__MXExt__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          dynamixel_workbench_msgs__msg__MXExt__fini(&data[i]);
        }
        free(data);
        return false;
      }
